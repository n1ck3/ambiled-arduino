...
/* legacy definitions, we now assume GRB, 800kHz */
#define MODE_RGB     0x00 // Wired for RGB data order
#define MODE_GRB     0x01 // Wired for GRB data order
#define MODE_COLMASK 0x01
#define MODE_KHZ400  0x00 // 400 KHz datastream
#define MODE_KHZ800  0x02 // 800 KHz datastream
#define MODE_SPDMASK 0x02
...
#endif // __arm__
/*---------------------------------------------------------------------------*/
ambiledStrip::ambiledStrip(uint8_t *buffer, uint16_t numpixels, uint8_t pin) : _pin(pin), _rgbbuf(buffer), _numpixels(numpixels)
#ifdef __AVR__
  , port(portOutputRegister(digitalPinToPort(pin))), pinMask(digitalPinToBitMask(pin))
#endif
{
  int i;
...
/*---------------------------------------------------------------------------*/
void
ambiledStrip::set_pixel_rgb(uint16_t pixelnum, uint8_t r, uint8_t g, uint8_t b)
{
  _rgbbuf[pixelnum * 3    ] = r;
  _rgbbuf[pixelnum * 3 + 1] = g;
  _rgbbuf[pixelnum * 3 + 2] = b;
}
/*---------------------------------------------------------------------------*/
uint16_t
ambiledStrip::no_of_pixels(void)
{
  return _numpixels;
}
/*---------------------------------------------------------------------------*/
void
ambiledStrip::update_from_buffer(uint8_t *buffer, uint16_t numpixels)
{
  if(buffer == NULL || numpixels == 0) {
    // argument error
    return;
  }
}
/*---------------------------------------------------------------------------*/
// static const uint8_t type;// Pixel flags (400 vs 800 KHz, RGB vs GRB color)
// static const uint8_t pin;// Output pin number
// static uint8_t *pixels; // Holds LED color values (3 bytes each)
// static uint8_t brightness;
// static const uint16_t numLEDs; // Number of RGB LEDs in strip

/* this function is for the most parts untouched */
void
ambiledStrip::update(void)
{
/* we will be running this mode, the original allows for more modes, we only run these on the WS2812B LEDs */
#define MODE_DEFAULT (MODE_KHZ800 | MODE_GRB)
  uint16_t numBytes = _numpixels * 3; // Size of 'pixels' buffer below
  uint8_t *pixels = _rgbbuf;
  uint8_t pin = _pin;
  uint8_t type = MODE_DEFAULT;
  if(pixels == NULL) {
    return;
  }
...
// 8 MHz(ish) AVR ---------------------------------------------------------
  ...
  if((type & MODE_SPDMASK) == MODE_KHZ800) { // 800 KHz bitstream
